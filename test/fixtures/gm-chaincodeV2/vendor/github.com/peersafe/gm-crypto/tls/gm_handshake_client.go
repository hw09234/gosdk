...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		fmt.Println("not first handshake")
	...
	if ka, ok := keyAgreement.(*eccSM2KeyAgreement); ok {
	...
	if certRequested {
		...
		certMsg.certificates = chainToSend.Certificate //signcert
		if len(c.config.Certificates) >= 2 {
			certMsg.certificates = append(certMsg.certificates, c.config.Certificates[1].Certificate[0]) // enccert
		}
		hs.finishedHash.Write(certMsg.marshal())
